import { _decorator, Component, PhysicsContact, PhysicsCollider } from 'cc';
const { ccclass, property } = _decorator

import Scene18Game from "./Scene18Game"

@ccclass('FruitsComponent')
export default class FruitsComponent extends Component {
...
//      // 小芝麻或大西瓜不处理碰撞检测
  ...
  update(dt: number) {
//    // 当前顶部水果 || 游戏已经结束 => 不处理
        // if (this.isHero || Scene18Game.instance.isGameOver()) {
        // return
        // }

        // let top = this.node.y + this.node.height / 2
        // if (Scene18Game.instance.isReachWarningLine(top)) {
        // this.overCount++
//      // 一直超过警戒线 1.5s 则游戏结束（update 方法 1s 执行 60 次，1.5s 执行 90 次）
        // if (this.overCount > 90) {
        // Scene18Game.instance.changeToGameOver()
        // }
        // } else {
        // this.overCount = 0
        // }
...
//      // 标记为不处理碰撞检测，避免另一个被碰撞的相同的水果也处理这段逻辑，导致合成两个新水果
      ...
      Scene18Game.instance.composeFruits(this, other)
    }
  }
//  // 只在两个碰撞体结束接触时被调用一次
  onEndContact(
