import { _decorator, Component, ToggleContainer, EditBox, Event, Toggle } from 'cc';
...
export default class Scene2 extends Component {
  ...
  onClickToggleContainer(target: Toggle, data: string) {
        // cc.log(target)
        // if (data === "men") {
        // cc.log("选择：男")
        // } else if (data === "women") {
...
//    // Toggle.toggleGroup 这个属性是可选的。如果这个属性为 null，则 Toggle 是一个 CheckBox，否则，Toggle 是一个 RadioButton
  ...
  onClickFruits(target: Toggle, data: string) {
        // if (target.isChecked) {
  ...
  editingDidBegan(target: EditBox, data: string) {
        // cc.log("editingDidBegan", target, data)
  }
  textChanged(text: string, target: EditBox, data: string) {
        // cc.log("textChanged", text, target, data)
  }
  editingDidEnded(target: EditBox, data: string) {
        // cc.log("editingDidEnded", target, data)
  }
  editingReturn(target: EditBox, data: string) {
